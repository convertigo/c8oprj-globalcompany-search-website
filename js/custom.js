...
//		use_siteclipper_plugin : "true", /** true/false : use the iframe encapsulation for siteclipper request */
		xsl_side : "server" /** client/server : force the side of the xsl transformation */
	}
});
...
//	if (data.__sequence == "searchCustomerAdvanced") {
//		query = "name LIKE '%" + data.name + "%' AND firstname LIKE '%" + data.firstname + "%' AND company LIKE '%" + data.company + "%'";
//		data.query = query;
//	}
//	return true;
//});
...
C8O.addHook("init_finished", function (data) {
	if (C8O.isUndefined(data.__sequence)) {
		C8O.waitHide();
		return false;
	} else {
		return true;
