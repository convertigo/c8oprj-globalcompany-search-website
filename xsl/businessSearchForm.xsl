					...
					<xsl:choose>
						<xsl:when test="authenticated">
							...
							<p>
						        ...
						        <a href=".cxml?__sequence=searchBusinessAdvancedForm" id="advancedSearch">Advanced search</a>	
							</p>
						
							<!-- search form -->
							<form id="businessSearch" action=".cxml" method="POST">
								<p><span>Keyword: </span><input type="text" id="businessSearch_keyword" name="keyword"/></p>
								<input type="submit" id="businessSearch_button" name="button" value="Search"/>
								<input type="hidden" id="businessSearch_sequence" name="__sequence" value="searchBusiness"/>
								<input type="hidden" id="businessSearch_start" name="start" value="0"/>
					        </form>
					        <p class="info">This keyword is searched among the project or company names.</p>
						</xsl:when>
						<xsl:otherwise>
