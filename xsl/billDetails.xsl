					...
					<xsl:choose>
						<xsl:when test="authenticated">
							<p>
								...
								<!-- Lien back to list -->
								| <a href=".cxml?{inputVars/backQuery}" id="backToBusinessList">Back to business list</a>
								
							</p>
							
							<!-- Content title -->
							<h3>Details of bill "<xsl:value-of select="inputVars/idBill"/>"&#160;for project "<xsl:value-of select="inputVars/projectName"/>"</h3>
...
<!-- content template for bill data -->
<xsl:template match="bill">
	<div id="bill_{@id}" class="billResult">
		<p class="date"><span class="title">Date: </span><xsl:value-of select="date"/></p>
		<p class="company"><span class="title">Company: </span><xsl:value-of select="/document/inputVars/company"/></p>
		<p class="address"><span class="title">Address: </span><xsl:value-of select="address"/>,&#160;<xsl:value-of select="postCode"/>&#160;<xsl:value-of select="city"/>,&#160;<xsl:value-of select="country"/></p>
		<p class="contact"><span class="title">Contact: </span><xsl:value-of select="contact"/></p>
		<p class="phone"><span class="title">Phone number: </span><xsl:value-of select="phone"/></p>
	</div>
	<br/>
</xsl:template>
